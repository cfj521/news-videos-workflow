...
1. 「发布管理」→「+ 添加平台」选 **抖音**，只需填一个**名称**（区分账号用，如 `抖音主号`）。无需填账号——登录靠扫码。
2. 点账号卡片的「扫码登录」，用抖音 App 扫弹出的二维码。
3. 卡片显示「已登录」后即可在任务里选该账号发布。登录态按账号自动存于仓库根 `publish_cookies/`，失效后重新扫码即可。
...
1. 「发布管理」→「+ 添加平台」选 **快手**，只需填一个**名称**（区分账号用）。无需填账号——登录靠扫码。
2. 点账号卡片的「扫码登录」，用快手 App 扫弹出的二维码。
3. 卡片显示「已登录」后即可在任务里选该账号发布。登录态同样按账号自动存于 `publish_cookies/`，失效后重新扫码。
