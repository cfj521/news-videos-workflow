...
def _isolate_store_paths(tmp_path, monkeypatch):
    """全局隔离：把 model_providers.yaml / publish_targets.yaml / news_sources.yaml
    重定向到每测试临时目录，防止任何测试（尤其经 /api/settings → save_settings、
    /api/publishers → targets_store、/api/sources → sources_store）写到仓库根的真实文件。
    自行 monkeypatch 这些路径的测试会在其测试体内覆盖此值。"""
    monkeypatch.setattr(
