...
export interface AppSettings {
  ...
  cover: { enabled: boolean; image: string; title_template: string; subtitle: string; narration: string; font_size: number; text_color: string; silent_duration: number };
  // 封面预设库（存 config.yaml）：active 选中下标 + 固定 3 个预设
  cover_presets: { active: number; presets: { name: string; values: { enabled: boolean; image: string; title_template: string; subtitle: string; narration: string; font_size: number; text_color: string; silent_duration: number } }[] };
